...
| packages/file-watcher | **Event Storm / Throttling Gap** | Chokidar can emit thousands of rapid file change events during bulk operations, causing cascading MarkdownGenerator writes and CPU spikes. | Implement debounced event batching with a 500ms coalescing window. Add a circuit breaker pattern that pauses watching after 100 consecutive events until the queue drains. |
| packages/vector-search | **Qdrant Connection Pool Exhaustion** | Unbounded concurrent semantic search requests can exhaust the Qdrant connection pool, causing 503 errors under load. | Implement a request queue with backpressure using a **Piscina** worker pool. Limit concurrent embeddings to 10 requests with exponential backoff retry for rate-limited calls. |
| packages/cloud-sync | **Partial Sync State Inconsistency** | Network interruptions during cloud sync can leave memory files in a half-encrypted state, corrupting the local vault. | Implement atomic sync transactions with two-phase commit. Write to a temporary staging directory first, then perform atomic rename operations. Add checksum validation on all transferred files. |
| apps/api | **Unauthorized Memory Access** | The Fastify API exposes memory file endpoints without request origin validation, allowing cross-project data leaks via loopback. | Implement **JWT-signed project tokens** with audience claims. Require a valid token matching the project's .ghost/config.json for all memory read/write endpoints. |
| apps/extension | **IPC Message Serialization Limits** | VS Code extension IPC has an 8MB message size limit; large memory snapshots cause extension host crashes. | Implement chunked streaming for memory file transfers. Split snapshots into 1MB chunks with sequence numbers and checksums. Reassemble on the receiving end before processing. |
